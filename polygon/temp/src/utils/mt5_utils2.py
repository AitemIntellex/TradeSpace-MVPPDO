...
def get_historical_account_data():
    ...
    start_date = today - timedelta(
        days=365
    )  # Предположим, что мы получаем данные за последний год

    historical_data = []
    rates = mt5.history_deals_get(start_date, today)

    if rates is None:
        raise Exception(f"Не удалось получить историю сделок: {mt5.last_error()}")

    # Сначала группируем сделки по датам
    balance = 0
    equity = 0
    for deal in rates:
        if deal.type == mt5.DEAL_TYPE_BALANCE:
            balance += deal.profit
        equity = (
            balance  # В нашем случае считаем equity равным балансу без открытых позиций
        )

        # Формируем список с историческими данными
        historical_data.append(
            {"date": deal.time, "balance": balance, "equity": equity}
        )
