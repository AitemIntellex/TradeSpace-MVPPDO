...
def calculate_pivot_points(symbol, timeframe, num_values=64):
    """
...
def calculate_ote(symbol, timeframe, trend="up", bars=500, local_bars=100):
    """
...
def calculate_fibonacci_levels(symbol, timeframe, trend="up", bars=500, local_bars=100):
    """
...
def calculate_atr(symbol, timeframe, period=14, num_values=3):
    ...
    return (
        [round(value, 5) for value in atr.iloc[-num_values:].tolist()]
        if not atr.empty
        else [None] * num_values
    )
...
def check_vwap(symbol, timeframe, period=100, num_values=3):
    df = get_rates_dataframe(symbol, timeframe, period + num_values - 1)
    if df is None:
        return [None] * num_values
...
def get_indicators_data(symbol, timeframe, trend="up", num_values=64):
    indicators = {}
    logging.info(
    ...
    try:
        ...
        logging.exception(f"Ошибка ATR: {e}")
    ...
    # Stochastic
    try:
        stochastic_k_values, stochastic_d_values = calculate_stochastic(
            symbol, timeframe, num_values=num_values
        )
        indicators["stochastic_k"] = stochastic_k_values
        indicators["stochastic_d"] = stochastic_d_values
    except Exception as e:
        logging.exception(f"Ошибка Stochastic: {e}")
        indicators["stochastic_k"] = [None] * num_values
        indicators["stochastic_d"] = [None] * num_values
