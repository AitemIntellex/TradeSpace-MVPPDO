...
def analyze_with_openai(result):
    ...
    messages = [
        {
            "role": "system",
            "content": "Вы — финансовый аналитик и трейдер, и ваша задача — улучшить свое представление, добавив четкую структуру, визуализацию данных и конкретные прогнозы.",
        },
        {
            "role": "user",
            "content": (
                f"Проанализируй следующие данные для {result['symbol']}:\n"
                f"Текущая цена: {result.get('current_price', 'N/A')}\n"
                f"Зона предложения: {result.get('supply_zone', 'N/A')}, Зона спроса: {result.get('demand_zone', 'N/A')}\n"
                f"RSI: {result.get('rsi', 'N/A')}\n"
                f"MACD: {result.get('macd', 'N/A')}, Signal: {result.get('signal', 'N/A')}\n"
                f"Bollinger Bands: Верхняя: {result.get('upper_band', 'N/A')}, Нижняя: {result.get('lower_band', 'N/A')}\n"
                f"ATR: {result.get('atr', 'N/A')}\n"
                f"Стохастик: K: {result.get('stochastic', {}).get('K', 'N/A')}, D: {result.get('stochastic', {}).get('D', 'N/A')}\n"
                f"VWAP: {result.get('vwap', 'N/A')}\n"
                f"CCI: {result.get('cci', 'N/A')}\n"
                f"MFI: {result.get('mfi', 'N/A')}\n"
                f"Уровни Фибоначчи: {result.get('fibonacci_levels', 'N/A')}\n"
                f"Канал регрессии: Верхний уровень: {result.get('regression_channel', {}).get('upper_channel', 'N/A')}, Нижний уровень: {result.get('regression_channel', {}).get('lower_channel', 'N/A')}, Наклон: {result.get('regression_channel', {}).get('slope', 'N/A')}, Пересечение оси Y: {result.get('regression_channel', {}).get('intercept', 'N/A')}\n"
                f"На основе этих данных дай рекомендации, включая открытие позиции, уровни стоп-лосса и тейк-профита, или конкретную цену для отложенного ордера."
            ),
        },
    ]
    ...
    try:
        response = openai.ChatCompletion.create(
            model="gpt-4",  # Используем chat-модель GPT-4
            messages=messages,
            max_tokens=500,
        )
...
def get_indicators_data(symbol, timeframe, trend="up"):
    ...
    try:
        ...
        if sma is not None and upper_band is not None and lower_band is not None:
            indicators["sma"] = sma
    ...
    try:
        ...
        if stochastic_k is not None and stochastic_d is not None:
            indicators["stochastic"] = {"K": stochastic_k, "D": stochastic_d}
    except Exception as e:
    ...
    try:
        ...
        if regression_channel is not None:
