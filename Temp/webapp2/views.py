...
def technical_analysis_indicators(request):
    ...
    # 7) Словари со значениями по умолчанию
    default_structure = {"trend": "Нет данных", "support": None, "resistance": None}
    default_indicators = {
        "atr": [],
        "mfi": [],
        "cci": [],
    ...
    default_pivot_points = [
        {"pivot": None, "pp_resistance": [None] * 3, "pp_support": [None] * 3}
    ]
    default_fibonacci = {"fib_levels": {}, "local_high": None, "local_low": None}

    # 8) Подключение к MT5 и сбор данных
    try:
        initialize_mt5()
        try:
            selected_pair_tick = get_currency_tick(symbol)
            ...
            for label, tf in timeframes.items():
                ...
                fibonacci_data = calculate_fibonacci_levels(
                    symbol, tf, bars=500, local_bars=100
                ) or dict(default_fibonacci)

                # 8.5) Структура рынка с экстремумами
                ms = identify_market_structure(symbol, tf) or dict(default_structure)
                ms["trend_color"] = get_color_for_trend(ms.get("trend", "Нет данных"))

                # Добавляем абсолютные и локальные экстремумы
    ...
    # Создание объединённых данных для шаблона
        ...
        merged_data_by_timeframe.append(
            {
                ...
                "market_structure": market_structure_by_timeframe.get(timeframe, {}),
            }
        )
